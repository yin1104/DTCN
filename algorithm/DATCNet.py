...
class DoubleScaleATCNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.global_block(x)  # [2, 8, 1, 250]
        x2 = self.local_block(x)
        x = torch.cat((x1, x2), 1)  # [2, 16, 1, 250]
        x = self.atten(x)

        x = torch.squeeze(x)
        x = self.tcn(x)
        x = torch.unsqueeze(x, 2)  # [2, 16, 1, 250]
        x = self.avgpool(x)
        x = self.dropout(x)  # [2, 16, 1, 31]
        x = torch.flatten(x, start_dim=1)  # [2, 672]
        x = self.fc(x)
        return x
...
def test():
    ...
    x = torch.randn(2, 11, 125)
    print("Input shape:", x.shape)
    y = model(x)
    print("Output shape:", y.shape)  # torch.Size([1, 250, 16])
    print(model)
    ...
    # print(torch.squeeze(model.atten.weight[0]))

    # summary(model, (11, 250))
